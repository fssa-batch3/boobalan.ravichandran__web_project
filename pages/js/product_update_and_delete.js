...
findDetails.addEventListener("click", (event) => {
  ...
  for (const company of companies) {
    ...
    const product = details.find((item) => item.product_unique_id === uniqueId);
    if (product) {
      ...
      document
        ...
        .addEventListener("click", (event) => {
          ...
          const productPrice = actualPriceInput.value;
          const priceDiscount = discountInput.value;
          const productWeight = productWeightInput.value;
...
document.getElementById("delete_details").addEventListener("click", (event) => {
  ...
  for (const company of companies) {
    ...
    if (details) {
      const productIndex = details.findIndex(
        (item) => item.product_unique_id === uniqueId
      );
      // rest of the code
